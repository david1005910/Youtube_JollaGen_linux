...
const generateScriptSingle = async (
  ...
  return retryGeminiRequest("Script Generation", async () => {
    ...
    if (finishReason === 'MAX_TOKENS' || (finishReason as string) === 'STOP_TRUNCATED') {
 ...
 * @param stylePrompt - 화풍 프롬프트 (서버 호출 시 클라이언트에서 전달, 없으면 localStorage 폴백)
 */
export const generateImageForScene = async (
  ...
  const resolvedStylePrompt = stylePrompt !== undefined
    ? stylePrompt
    : (typeof localStorage !== 'undefined' ? getSelectedGeminiStylePrompt() : '');
  const geminiStylePrompt = hasStyleRef ? undefined : resolvedStylePrompt;
...
Focus on: color palette, brush strokes, lighting, overall mood.`
        ...
        // (스타일 참조 이미지 우선 원칙)
        if (!hasStyleRef) {
          const styleToApply = stylePrompt !== undefined
            ? stylePrompt
            : (typeof localStorage !== 'undefined' ? getSelectedGeminiStylePrompt() : '');
          if (styleToApply) {
            parts.push({
              text: `[ART STYLE INSTRUCTION]
Apply this art style: ${styleToApply}
Ensure the entire image consistently follows this visual style.`
            });
            console.log(`[Image Gen] Gemini 화풍 적용: ${styleToApply.slice(0, 50)}...`);
          }
        }
