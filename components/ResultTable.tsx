...
interface TableRowProps {
  ...
  onOpenVideo?: (src: string, title: string, durationSec: number) => void;
}

const TableRow: React.FC<TableRowProps> = memo(({ row, index, isAnimating, onRegenerateImage, onGenerateAnimation, onOpenVideo }) => {
  return (
        ...
        <div className="space-y-3">
          ...
          {row.analysis?.composition_type && (
            <div className="flex flex-wrap gap-1">
              ...
              {(row.analysis as any).sentiment && (
                <span className={`text-[7px] font-black px-1.5 py-0.5 rounded border uppercase ${
                  (row.analysis as any).sentiment === 'POSITIVE' ? 'text-green-400 bg-green-400/5 border-green-400/20' :
                  (row.analysis as any).sentiment === 'NEGATIVE' ? 'text-red-400 bg-red-400/5 border-red-400/20' :
                  'text-slate-400 bg-slate-400/5 border-slate-400/20'
                }`}>{(row.analysis as any).sentiment}</span>
              )}
            </div>
          )}
        </div>
          ...
          {row.status === 'generating' ? (
            ...
            <>
              ...
              <div className="absolute inset-0 bg-slate-950/80 opacity-0 group-hover/img:opacity-100 transition-all flex items-center justify-center gap-1.5">
                {/* 전체화면 미리보기 */}
                <button
                  onClick={() => onOpenVideo?.(row.videoData!, `씬 ${row.sceneNumber}`, row.videoDuration ?? 5)}
                  className="p-2 rounded-lg bg-cyan-500/30 hover:bg-cyan-500/60 border border-cyan-400/40 text-cyan-300 transition-all"
                  title="전체화면으로 보기"
                >
